...
class AnnulusGridOriginal {
	...
	static void setGridSize(int gridSize) {
		AnnulusGridOriginal.gridSize = gridSize;
	}
	
	static void setMin(double min) {
		AnnulusGridOriginal.minX = -min;
		AnnulusGridOriginal.minY = -min;
		
	}
	
	static void setMax(double max) {
		AnnulusGridOriginal.maxX = max;
		AnnulusGridOriginal.maxY = max;
	}
	...
	static double getColRandom(int col) {
		AnnulusGridOriginal.col = AnnulusGridOriginal.col ++;
	...
	static double getRowRandom(int row) {
		AnnulusGridOriginal.row = AnnulusGridOriginal.row ++;
	...
	static int iterator() {
		...
		for(int col = 0; col < gridSize - 1; col ++) {
			...
			for(int row = 0; row < gridSize - 1; row ++) {
				...
				if(test < AnnulusOriginal.rad1Sq() && test > AnnulusOriginal.rad2Sq()) {
					counter ++;
				}
			}
		}
