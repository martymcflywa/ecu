...
%$U$ is calculated for each pair of independent samples, where $a$ is the first sample, $b$ is the second sample, and $R$ is the sum of the ranks:

%$$U_a = N_aN_b + \frac{N_a(N_a + 1)}{2} - R_a$$

%$$U_b = N_aN_b + \frac{N_b(N_b + 1)}{2} - R_b$$
%\\
%The lower value between the two is the observed $U$ \citep[p. 463]{Coolican2014}. The results of this test are displayed in Table 4.
