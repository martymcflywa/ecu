...
When winning, the strategy is as follows. If close or near, use the minimum speed to prevent overshooting the enemy, and also allow the enemy to overtake so that it can be followed. If near, use maximum speed to catch up to get within a more lethal range for the cannon. When the score is even, the strategy is to be more conservative with energy and only used medium speed if far. Otherwise, use the minimum speed if close or near. When losing, the strategy is to fly fast away from the enemy if close, and fly at a moderate speed if near. Otherwise use the minimum speed if far away.
