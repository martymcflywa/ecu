...
local jsonutil = require("modules.json.util")
local merge = jsonutil.merge
local util = require("modules.json.decode.util")

local decode_state = require("modules.json.decode.state")
...
for _,name in ipairs(modulesToLoad) do
	local mod = require("modules.json.decode." .. name)
